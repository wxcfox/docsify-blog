...
**处理异常**：try catch finally三部分组成。try包裹业务代码，catch用于捕获并处理某个类型的异常，finally用于回收资源。

**抛出异常**：当程序出现错误时,系统会自动抛出异常，Java也允许程序主动抛出异常，使用throw关键字向外抛出异常。

**异常跟踪栈**：程序运行时会发生一系列方法调用，从而形成方法调用栈。异常从发生异常的方法向外传播，首先传给该方法的调用者，再传给上层调用者，以此类推。若最终没有得到处理，则JVM会终止程序，并打印异常跟踪栈的信息。

## 13. 说说你对面向对象的理解

**三大基本特征**：封装、继承、多态。

**封装**：对象内部实现细节对外不公开，不允许外部程序直接访问对象内部信息，外部程序可通过该类提供的方法来实现对内部信息的操作和访问。有助于规范操作行为，提高代码可维护性。

**继承**：实现代码复用的重要手段，Java通过extends关键字实现继承。被继承的类称为父类，实现继承的类称为子类，一个子类只能有一个直接父类。

**多态**：多态的实现离不开继承，在设计程序时，我们可以将参数的类型定义为父类型，在调用程序时，可以通过实际情况引入父类型的某个子类型的实例，这样就实现了多态。
