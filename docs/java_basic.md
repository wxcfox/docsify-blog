...
通过自动装箱、自动拆箱，**简化基本数据类型变量和包装类之间的转换过程**。有了包装类，所有类都可以继承object类。

## 12. 请你说说Java的异常处理机制

**作用**：让程序具有容错性、健壮性，当业务代码发生异常，系统会创建一个Exception对象并交给JVM，然后由JVM寻找可以处理这个异常的catch块，如果没有找到，那么运行环境终止、Java程序退出。

**组成**：处理异常，抛出异常，异常跟踪栈。

&ensp;&ensp;**处理异常**：try catch finally三部分组成。try包裹业务代码，catch用于捕获并处理某个类型的异常，finally用于回收资源。

&ensp;&ensp;**抛出异常**：当程序出现错误时,系统会自动抛出异常，Java也允许程序主动抛出异常，使用throw关键字向外抛出异常。

&ensp;&ensp;**异常跟踪栈**：程序运行时会发生一系列方法调用，从而形成方法调用栈。异常从发生异常的方法向外传播，首先传给该方法的调用者，再传给上层调用者，以此类推。若最终没有得到处理，则JVM会终止程序，并打印异常跟踪栈的信息。

## 13. 说说你对面向对象的理解

**三大基本特征**：封装、继承、多态。

&ensp;&ensp;**封装**：对象内部实现细节对外不公开，不允许外部程序直接访问对象内部信息，外部程序可通过该类提供的方法来实现对内部信息的操作和访问。有助于规范操作行为，提高代码可维护性。

&ensp;&ensp;**继承**：实现代码复用的重要手段，Java通过extends关键字实现继承。被继承的类称为父类，实现继承的类称为子类，一个子类只能有一个直接父类。

&ensp;&ensp;**多态**：多态的实现离不开继承，在设计程序时，我们可以将参数的类型定义为父类型，在调用程序时，可以通过实际情况引入父类型的某个子类型的实例，这样就实现了多态。

## 14. 请你说说重载和重写的区别，构造方法能不能重写

**重载（Overloading）：**
- 定义：同一类中的多个方法，方法名相同但参数列表不同。
- 作用：重载的目的是为了提供更多的方法重载选择，使方法可以根据不同的参数类型或个数进行适当的操作。

**重写（Overriding）：**
- 定义：在子类中定义一个与父类中具有相同名称、参数列表和返回类型的方法。
- 作用：重写的目的是为了实现多态性，即在子类中重新定义父类的方法，以便子类根据自身的需要进行特定实现。

## 15. 请介绍一下访问修饰符

| 修饰符   | 类内部访问 | 同一包内访问 | 子类访问 | 其他包内访问 |
|----------|------------|--------------|---------|--------------|
| public   | ✔          | ✔            | ✔       | ✔            |
| protected| ✔          | ✔            | ✔       | ✘            |
| default  | ✔          | ✔            | ✘       | ✘            |
...
**泛型的作用：**
- 类型安全：编译期进行类型检查，可以减少运行期可能会发生的错误。
- 代码重用：定义泛型，可以适用于不同的数据类型，避免重复代码。
